				  ...
				  <div>
					...
					<div class="max-w-md space-y-3">
					  ...
					  <a wire:navigate class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-transparent bg-blue-600 text-white hover:bg-blue-700 focus:outline-none focus:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none" href="/add/product">
						<svg class="shrink- size-4" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14"/><path d="M12 5v14"/></svg>
						Add Product
					  </a>
					</div>
				  </div>
					  ...
					  @if (count($products) > 0)
						  ...
						  <tr wire:key="{{$product->id}}">
							  ...
							  <td class="size-px whitespace-nowrap">
								<div class="px-6 py-3">
								  <span class="py-1 px-1.5 inline-flex items-center gap-x-1 text-xs font-medium dark:text-neutral-500">
									${{$product->price}}
								  </span>
								</div>
							  </td>
							  ...
							  <td class="size-px whitespace-nowrap">
								<div class="px-6 py-1.5">
								  <a wire:navigate wire:click="delete({{$product->id}})" wire:confirm.prompt="Are you sure?\n\nType DELETE to confirm|DELETE" class="inline-flex items-center gap-x-1 text-sm  decoration-2 hover:underline focus:outline-none focus:underline font-medium dark:text-red-500" href="">
...
<div class="px-6 py-4 grid gap-3 md:flex md:justify-between md:items-center border-t border-gray-200 dark:border-neutral-700">
    <div class="flex gap-2">
      <label class="w-32 text-sm font-medium text-blue-600">Per Page</label>
    ...
    <div class="flex items-center justify-center space-x-2">
      @if ($products->onFirstPage())
        <span class="px-3 py-1 text-gray-400 bg-gray-200 rounded cursor-not-allowed">Previous</span>
      @else
        <a href="{{ $products->previousPageUrl() }}" class="px-3 py-1 text-white bg-blue-600 rounded hover:bg-blue-700">Previous</a>
      @endif
      ...
      @foreach ($products->links()->elements as $element)
        ...
        @if (is_array($element))
          @foreach ($element as $page => $url)
            ...
            @else
              <a href="{{ $url }}" class="px-3 py-1 text-blue-600 bg-white border rounded hover:bg-gray-100">{{ $page }}</a>
            @endif
          @endforeach
        @endif
      @endforeach
      ...
      @if ($products->hasMorePages())
        <a href="{{ $products->nextPageUrl() }}" class="px-3 py-1 text-white bg-blue-600 rounded hover:bg-blue-700">Next</a>
      @else
        <span class="px-3 py-1 text-gray-400 bg-gray-200 rounded cursor-not-allowed">Next</span>
      @endif
    </div>
  </div>
  <!-- End Footer -->
