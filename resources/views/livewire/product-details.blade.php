<div>
    <div class="flex gap-5 p-20">
        <img src="{{ $product->image ? Storage::url($product->image) : asset('images/placeholder-image.jpg') }}"
        alt="product-images"
       class="rounded-t-lg object-cover w-[300px] h-[180px]">
        <div>
            <h2 class="line-clamp-2 p-1 font-medium text-2xl">{{ $product->name }}</h2>
            <h2 class="line-clamp-4 p-1 text-gray-500">{{ $product->description }}</h2>
